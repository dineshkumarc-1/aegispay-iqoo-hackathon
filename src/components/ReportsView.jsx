...
import { 
  Printer, Download, Copy, Check, ShieldAlert, 
  FileText, Building2, CheckCircle2, Lock, ShieldCheck, Scale, 
  Hash, ExternalLink, Calendar, MapPin, ArrowDownToLine
} from 'lucide-react';
...
export default function ReportsView({ onOpenDeck, onOpenArch }) {
  ...
  const handleCopy = () => {
    const text = `GOVERNMENT OF INDIA • MINISTRY OF HOME AFFAIRS (MHA)
...
DATE & TIME: ${firDetails.dateTime}
COMPLAINANT: ${firDetails.complainant.name} (${firDetails.complainant.store})
FLAGGED MULE VPAS: quick_refund_x98234@ibl, cashback_settle_claim@ybl
FLAGGED MULE ACCOUNTS: SBI 30918239012, HDFC 5010048192301
APPLIED SECTIONS: IT Act 66D, 66C, 43 | BNS 318(4), 316(2)
SECTION 65B EVIDENCE SHA-256 HASH: ${firDetails.sha256EvidenceHash}`;
...
`;
  ...
  return (
    <div className="space-y-4 pb-8 text-xs text-slate-800 font-sans leading-relaxed">
      
      {/* Top Action Bar (Hidden in Print/PDF) */}
      <div className="no-print p-3 bg-white border border-slate-200 rounded-2xl flex flex-wrap items-center justify-between gap-2 shadow-2xs">
        <div className="flex items-center gap-1.5">
          <Scale className="w-4 h-4 text-rose-600 shrink-0" />
          <span className="font-bold text-slate-900 text-xs truncate">
            Official 1930 Cybercrime Police FIR
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={handleCopy}
            className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-[11px] transition cursor-pointer flex items-center gap-1 active:scale-95"
          >
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-xs transition cursor-pointer flex items-center gap-1.5 shadow-md active:scale-95"
          >
            <Printer className="w-4 h-4 text-white" />
            <span>Save / Download Official PDF</span>
          </button>
        </div>
      </div>

      {/* Main Printable Legal FIR Document Container */}
      <div id="printable-fir-report" className="space-y-4">
        
        {/* Official Government NCRP Header Badge */}
        <div className="p-4 rounded-2xl bg-white border-2 border-slate-800 shadow-xs space-y-2.5 text-center">
          ...
          <div className="pt-2 border-t border-slate-200 grid grid-cols-2 gap-2 text-[10px] text-left font-mono">
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200">
              <span className="text-slate-400 block text-[9px] uppercase font-bold">Acknowledgement No</span>
              <strong className="text-slate-900 truncate block">{firDetails.acknowledgementNo}</strong>
            </div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200">
              <span className="text-slate-400 block text-[9px] uppercase font-bold">Jurisdiction</span>
              <strong className="text-slate-900 truncate block">Bengaluru City Police</strong>
            </div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200">
              <span className="text-slate-400 block text-[9px] uppercase font-bold">Incident Timestamp</span>
              <strong className="text-slate-900 truncate block">{firDetails.incidentDate}</strong>
            </div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200">
              <span className="text-slate-400 block text-[9px] uppercase font-bold">Evidence Admissibility</span>
              <strong className="text-emerald-700 truncate block">Sec 65B Certified ✓</strong>
            </div>
          </div>
        </div>
      ...
      {/* Extra Compliance Actions (Hidden in Print/PDF) */}
      <div className="no-print p-4 rounded-2xl bg-white border border-slate-200 space-y-2.5">
        <div className="font-bold text-slate-900 text-xs">Additional Compliance Proofs</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button
            onClick={downloadDPDPAudit}
            className="py-2 px-3 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 font-bold text-[11px] transition cursor-pointer flex items-center justify-center gap-1.5"
          >
            <Download className="w-3.5 h-3.5" />
            <span>Download DPDP Privacy Audit</span>
          </button>

          <button
            onClick={onOpenDeck}
            className="py-2 px-3 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 font-bold text-[11px] transition cursor-pointer flex items-center justify-center gap-1.5"
          >
            <FileText className="w-3.5 h-3.5" />
            <span>Open 10-Slide Pitch Deck</span>
          </button>
        </div>
      </div>
